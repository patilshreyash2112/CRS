    ...
    useEffect(() => {
        const fetchComplaints = async () => {
            try {
                ...
                // Fetch additional details and filter complaints with statusName 'assigned'
                const updatedComplaints = await Promise.all(
                    data
                        .filter(complaint => complaint.statusName.toLowerCase() === 'assigned')
                        .map(async (complaint) => {
                        ...
                        <tbody>
                            {complaints.map((complaint, index) => (
                                <tr key={complaint.id} onClick={() => handleShow(complaint)} style={{ cursor: 'pointer' }}>
                                    ...
                                    <td>{complaint.assignedPoliceStationName}</td>
                                    <td>{complaint.assignedPoliceOfficerName === 'Admin' ? 'Not Assigned' : complaint.assignedPoliceOfficerName}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                        ...
                        <Container>
                            <Card>
                                <Card.Body>
                                    ...
                                    <Card.Text><strong>Assigned Police Station:</strong> {selectedComplaint.assignedPoliceStationName}</Card.Text>
                                    <Card.Text>
